...
public class ScriptClientImpl implements ScriptClient {
    ...
    public Status startJoinScript(String query,
        ...
        Operation<Result<ScriptResult>> t = this.startScript(query, params, settings).join();
        Result<ScriptResult> t1 = t.getValue();
        ScriptResult t2 = t1.getValue();

        return t2.waitForResult();
    }
    ...
    @Override
    public CompletableFuture<Operation<Result<ScriptResult>>> findScript(String operationId, FindScriptSettings settings) {
        GrpcRequestSettings options = makeGrpcRequestSettings(settings);

        return scriptRpc.getOperation(operationId, options).
                thenApply(
                        op -> op.transform(
                                r -> r.map(w -> ScriptResult.ScriptResult1(op))));
    }

    @Override
    public CompletableFuture<Operation<Result<ScriptResult>>> startScript(String query,
                                                                          Params params,
                                                                          ExecuteScriptSettings settings) {
