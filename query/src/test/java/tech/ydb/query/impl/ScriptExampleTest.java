...
public class ScriptExampleTest {
    ...
    public void findAndStartScript() {
        ...
        CompletableFuture<Operation<Result<ScriptResult>>> future = scriptClient.startScript(""
                        + "DECLARE $values AS List<Struct<"
                        + "  series_id: Uint64,"
                        + "  season_id: Uint64,"
                        + "  title: Text,"
        ...
        Operation<Result<ScriptResult>> operationScript = future.join();


        Operation<Result<ScriptResult>> operationScript1 = scriptClient.findScript(operationScript.getId(), FindScriptSettings.newBuilder().build()).join();


        Assert.assertEquals(operationScript1.getId(), operationScript.getId());

        Status status = Objects.requireNonNull(operationScript1.getValue()).getValue().waitForResult();
    ...
    public void fetchScript() throws ExecutionException, InterruptedException {
        ...
        Operation<Result<ScriptResult>> operationScript = future.join();
        Status status = Objects.requireNonNull(operationScript.getValue()).getValue().waitForResult();
        Assert.assertTrue(status.isSuccess());
